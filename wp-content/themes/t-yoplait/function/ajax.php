...
function ajax_filterposts_handler() {
    $ingredients = (isset($_POST['ingredients'])) ? $_POST['ingredients'] : false;
    $modele = (isset($_POST['modele'])) ? $_POST['modele'] : false;
    $temps = (isset($_POST['temps'])) ? $_POST['temps'] : false;
    ...
    if( $ingredients):
        $tax_query[] = array(
            array( 
                'taxonomy' => 'tax_ingredient', 
                'field' => 'id', 
                'terms' => $_POST['ingredients'] ,
                // 'operator' => 'AND'
            )
        );
    endif;
    ...
    $posts = '<div class="nopostfound mb-5 pb-2 text-center">Aucun résultat trouvé.</div>';
    ...
    if ( $data->have_posts() ) {
        ...
        <div id="pagination">
            <?php if ( $data->max_num_pages > 1 ) :?>
                <nav class="d-flex justify-content-center align-items-center pb-5" aria-label="Page navigation recettes">
